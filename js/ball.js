...
(function (MiniBillar) {
  class Ball extends Phaser.Group {
    constructor(game, ballRadius, n, ballObject) {
      ...
      this.ballImage = new Phaser.Image(
        game,
        0,
        0,
        "texture_atlas_1",
      ...
      if (this.ballType === MiniBillar.GameConstants.BALL_TYPE_SOLID) {
        this.ballImage.frameName =
          spriteSheetPrefix + this.n.toString() + ".png";
      } else {
        this.ballImage.frameName =
          spriteSheetPrefix + this.n.toString() + "_0.png";
      }
    ...
    update() {
      if (
        MiniBillar.PoolState.currentInstance.victoryLayer ||
        MiniBillar.PoolState.currentInstance.loseLayer
      ) {
        if (this.pocketTween) {
      ...
      if (this.pocketTween) {
        this.updateRotation(
          this.ballObject.velocity.x,
          this.ballObject.velocity.y,
          0
        );
      }
      super.update();
    ...
    rotateQuat(t, s, h, i, a) {
      ...
      let e = h / o;
      let l = i / o;
      let p = Math.sin(0.5 * a);
      let n = r * p;
      let d = e * p;
      let c = l * p;
      let M = Math.cos(0.5 * a);
      let P = t[0];
      let y = t[1];
      let H = t[2];
      let R = t[3];
    ...
    renderBall(q) {
      ...
      if (!(test > 0.499 || test < -0.499)) {
        ...
        if (this.ballType === MiniBillar.GameConstants.BALL_TYPE_STRIPED) {
          ...
          this.ballImage.frameName =
            "stripes_" + this.n + "_" + rotationIndex.toString() + ".png";
        }
        if (this.spotHolder) {
          const rotationZ =
            Math.atan2(
          ...
          if (
            rotationX < Billiard.Maths.PI_2 ||
            rotationX > 3 * Billiard.Maths.PI_2
          ) {
            if (
              rotationZ > Billiard.Maths.PI_2 &&
              rotationZ < 3 * Billiard.Maths.PI_2
            ) {
              this.spotHolder.y =
                this.circRad * Math.cos(rotationZ) * Math.sin(rotationX);
          ...
          if (scaleY < 0.1) {
            ...
            this.spotHolder.scale.set(0.8, scaleY * spotScale);
          }
